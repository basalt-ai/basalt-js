...
describe("API", () => {
	...
	test("does network request with endpoint-defined path", async () => {
		...
		const api = new Api(new URL("http://test/"), mockedNetwork, "some-api-key");
	...
	test.each([
		...
		"put",
		"delete",
	] as FetchMethod[])("does network request with endpoint-defined http method (%s)", async (method) => {
		mockedNetwork.fetch.mockImplementationOnce(() => ok({}));
		fakeEndpoint.decodeResponse.mockImplementationOnce(() => ok({}));
		fakeEndpoint.prepareRequest.mockImplementationOnce(() => ({
			path: "",
	...
	test("decodes fetch result using endpoint", async () => {
		...
		fakeEndpoint.decodeResponse.mockImplementationOnce(() =>
			ok({ some: "decoded-response" }),
		);
		mockedNetwork.fetch.mockImplementationOnce(() => ok({}));
	...
	test("forwards decoding error", async () => {
		...
		fakeEndpoint.decodeResponse.mockImplementationOnce(() =>
			err({ message: "Some decoding problem" }),
		);
		mockedNetwork.fetch.mockImplementationOnce(() => ok({}));
