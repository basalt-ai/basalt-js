...
describe("CreateDatasetItemEndpoint", () => {
	...
	test("positively decodes valid responses", () => {
		const result = CreateDatasetItemEndpoint.decodeResponse(
			fixtures.validResponse.body,
		);

		expect(result.error).toBeNull();
		expect(result.value?.datasetRow).toMatchObject(
			fixtures.validResponse.body.datasetRow,
		);
	});

	test("positively decodes responses with warnings", () => {
		const result = CreateDatasetItemEndpoint.decodeResponse(
			fixtures.responseWithWarning.body,
		);

		expect(result.error).toBeNull();
		expect(result.value?.warning).toBe(
			fixtures.responseWithWarning.body.warning,
		);
		expect(result.value?.datasetRow).toMatchObject(
			fixtures.responseWithWarning.body.datasetRow,
		);
	});
