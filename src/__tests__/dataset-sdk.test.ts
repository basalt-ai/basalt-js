...
import { ListDatasetsEndpoint, GetDatasetEndpoint, CreateDatasetItemEndpoint } from '../lib/endpoints'
...
describe('DatasetSDK', () => {
	...
	describe('list()', () => {
		...
		test('logs warning when response includes warning', async () => {
			...
			expect(mockLogger.warn).toHaveBeenCalledWith(expect.stringContaining('Some datasets are deprecated'))
		})
	...
	describe('addRow() with files', () => {
		...
		test('calls uploadFile for each FileAttachment', async () => {
			const file1 = new FileAttachment(Buffer.from('file1'), { filename: 'file1.txt' })
			const file2 = new FileAttachment(Buffer.from('file2'), { filename: 'file2.txt' })
		...
		test('handles mixed values (files and strings)', async () => {
			const file1 = new FileAttachment(Buffer.from('data'), { filename: 'file.txt' })
