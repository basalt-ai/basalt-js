...
describe("GetDatasetEndpoint", () => {
	test("slug is included in pathname", () => {
		const result = GetDatasetEndpoint.prepareRequest({
	...
	test("positively decodes valid responses", () => {
		const result = GetDatasetEndpoint.decodeResponse(
			fixtures.validResponse.body,
		);

		expect(result.error).toBeNull();
		expect(result.value?.dataset).toMatchObject(
			fixtures.validResponse.body.dataset,
		);
	});

	test("positively decodes dataset with empty rows", () => {
		const result = GetDatasetEndpoint.decodeResponse(
			fixtures.emptyRowsResponse.body,
		);

		expect(result.error).toBeNull();
		expect(result.value?.dataset).toMatchObject(
			fixtures.emptyRowsResponse.body.dataset,
		);
