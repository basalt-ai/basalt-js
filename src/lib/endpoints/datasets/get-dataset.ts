...
export default class GetDatasetEndpoint {
	...
	static decodeResponse(body: unknown): Result<Output> {
		if (body === null || typeof body !== "object") {
			return err({
				message: "Get Dataset: Failed to decode response (invalid body format)",
			});
		}
		...
		if (
			...
			return err({
				message: "Get Dataset: Failed to decode response (missing dataset)",
			});
		}
		...
		if (!("slug" in dataset) || typeof dataset.slug !== "string") {
			return err({
				message:
					"Get Dataset: Failed to decode response (missing dataset.slug)",
			});
		}
		...
		if (!("name" in dataset) || typeof dataset.name !== "string") {
			return err({
				message:
					"Get Dataset: Failed to decode response (missing dataset.name)",
			});
		}
		...
		if (!("columns" in dataset) || !Array.isArray(dataset.columns)) {
			return err({
				message:
					"Get Dataset: Failed to decode response (missing dataset.columns)",
			});
		}
		...
		if (!("rows" in dataset) || !Array.isArray(dataset.rows)) {
			return err({
				message:
					"Get Dataset: Failed to decode response (missing dataset.rows)",
			});
		}
