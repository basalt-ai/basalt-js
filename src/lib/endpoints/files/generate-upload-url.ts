...
export default class GenerateUploadUrlEndpoint {
	...
	static decodeResponse(body: unknown): Result<Output> {
		if (body === null || typeof body !== "object") {
			return err({
				message:
					"Generate Upload URL: Failed to decode response (invalid body format)",
			});
		}
		...
		if (!("uploadUrl" in body) || typeof body.uploadUrl !== "string") {
			return err({
				message:
					"Generate Upload URL: Failed to decode response (missing uploadUrl)",
			});
		}
		...
		if (!("fileKey" in body) || typeof body.fileKey !== "string") {
			return err({
				message:
					"Generate Upload URL: Failed to decode response (missing fileKey)",
			});
		}
		...
		if (!("expiresAt" in body) || typeof body.expiresAt !== "string") {
			return err({
				message:
					"Generate Upload URL: Failed to decode response (missing expiresAt)",
			});
		}
		...
		if (!("maxSizeBytes" in body) || typeof body.maxSizeBytes !== "number") {
			return err({
				message:
					"Generate Upload URL: Failed to decode response (missing maxSizeBytes)",
			});
		}
