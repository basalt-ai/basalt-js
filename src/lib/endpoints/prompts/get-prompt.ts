...
export default class GetPromptEndpoint {
	...
	static decodeResponse(body: unknown): Result<Output> {
		...
		if (!("text" in body.prompt) || typeof body.prompt.text !== "string") {
			return err({
				message: "Get Prompt: Failed to decode response (missing prompt.text)",
			});
		}

		if (
			!("version" in body.prompt) ||
			typeof body.prompt.version !== "string"
		) {
			return err({
				message:
					"Get Prompt: Failed to decode response (missing prompt.version)",
			});
		}

		if (
			!("systemText" in body.prompt) ||
			(typeof body.prompt.systemText !== "string" &&
				body.prompt.systemText !== undefined)
		) {
			return err({
				message:
					"Get Prompt: Failed to decode response (missing prompt.systemText)",
			});
		}
		...
		if (
			...
			return err({
				message: "Get Prompt: Failed to decode response (missing prompt.model)",
			});
		}
