...
export default class DatasetSDK implements IDatasetSDK {
	...
	}): AsyncResult<DatasetListResponse[]> {
	...
	async get(
		slug: string,
		options?: { kind?: import("../telemetry/types").ObserveKind },
	...
	private async _getDatasetWithCache(
		...
		if (fallback) {
			...
			this.logger.warn(
				`Basalt Warning: Failed to fetch dataset from API, using last result for "${options.slug}"`,
			);
			return ok(fallback);
		}
	 ...
	 * @returns A promise with the created dataset item response.
	 */
	private async _createDatasetItem(
		options: { slug: string } & CreateDatasetItemOptions,
	): AsyncResult<CreateDatasetItemResponse> {
