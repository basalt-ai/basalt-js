	...
	try {
		...
		if (!carrier.get("traceparent")) {
			...
			if (spanContext?.traceId && spanContext?.spanId) {
				...
				carrier.set(
					"traceparent",
					`00-${spanContext.traceId}-${spanContext.spanId}-${traceFlags}`,
				);
			}
		}
	} catch {
		...
		try {
			try {
				...
				if (response.status === 401) {
					return err(new Unauthorized({ url, message: json.error }));
				}

				if (response.status === 403) {
					return err(new Forbidden({ url, message: json.error }));
				}

				if (response.status === 404) {
					return err(new NotFound({ url, message: json.error }));
				}

				if (response.status === 422) {
					return err(new BadInput({ url, message: json.error }));
				}
